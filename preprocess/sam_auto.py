...
# 掩码质量评分配置选项
quality_settings = parser.add_argument_group("Quality Settings")
quality_settings.add_argument("--use-quality-score", action="store_true", help="Use quality score for mask sorting instead of predicted_iou.")
quality_settings.add_argument("--iou-weight", type=float, default=0.5, help="Weight for predicted_iou in quality score calculation.")
quality_settings.add_argument("--stability-weight", type=float, default=0.3, help="Weight for stability_score in quality score calculation.")
quality_settings.add_argument("--area-weight", type=float, default=0.2, help="Weight for area normalization in quality score calculation.")
...
def main(args: argparse.Namespace):
    ...
    for img_i in tqdm.tqdm(imgs):
        ...
        # 根据配置决定是否使用质量分数排序
        if args.use_quality_score:
            mask, _ = get_single_mask(
                anns, 
                quality_score=True, 
            ...
            mask, _ = get_single_mask(anns, metric='predicted_iou', descending=False)
